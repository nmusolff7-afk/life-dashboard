...
_CATALOG: tuple[ConnectorMeta, ...] = (
    ...
    ConnectorMeta(
        ...
        description='Steps, sleep, heart rate, HRV, active calories',
        category='fitness', kind='device_native', icon='❤️',
        ships_in_phase='c1',
        note='Read-only via Android Health Connect. Tap to grant permissions and sync today\'s metrics.',
        platforms=('android',),
    ),
    ConnectorMeta(
        provider='gmail',
    ...
    ConnectorMeta(
        ...
        description='Today\'s events, meeting density',
        category='time', kind='oauth', icon='📅',
        ships_in_phase='c1',
        note='Connect via Google OAuth. Events from yesterday + next 7 days import on connect.',
    ),
    ConnectorMeta(
        provider='outlook',
        ...
        category='time', kind='oauth', icon='📬',
        ships_in_phase='c1',
        note='Connect via Microsoft. Imports recent inbox + the next 7 days of calendar events.',
    ),
    ConnectorMeta(
        provider='plaid',
    ...
    ConnectorMeta(
        provider='strava',
        display_name='Strava',
        description='Recent runs, rides, and workouts (read-only)',
        category='fitness', kind='oauth', icon='🏃',
        ships_in_phase='c1',
        note='Connect via Strava OAuth. Activities from the last 90 days import into your workout history.',
    ),
    ConnectorMeta(
        provider='garmin',
    ...
    ConnectorMeta(
        ...
        display_name='Apple Screen Time',
        description='Screen-time patterns for the attention pillar',
        category='attention', kind='device_native', icon='🛡️',
        ships_in_phase='v1.1',
        note='Requires Apple distribution entitlement (~2 week lead). iOS only.',
        platforms=('ios',),
    ),
    ConnectorMeta(
        provider='android_usage_stats',
        display_name='Screen Time',
        description='Daily phone-usage minutes + top-app breakdown',
        category='attention', kind='device_native', icon='📱',
        ships_in_phase='c1',
        note='Reads from Android UsageStatsManager via custom Expo Module. You\'ll be sent to system Settings once to grant Usage Access.',
        platforms=('android',),
    ),
    ConnectorMeta(
        provider='location',
        display_name='Location',
        description='Where you spend time (foreground samples for v1)',
        category='time', kind='device_native', icon='📍',
        ships_in_phase='c1',
        note='Foreground-only sampling for v1. Home/work/gym clustering ships in v1.1 alongside background location.',
    ),
)
