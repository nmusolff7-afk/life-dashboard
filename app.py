...
# ═══════════════════════════════════════════════════════════════════════════
# TEMPORARY — Clerk→Flask user-linking migration endpoints.
# REMOVE AFTER USE.
#
# Used once to wire a specific Clerk identity (user_xxx) to a pre-existing
# Flask user row on Railway, so a returning user gets matched to their
# historical meal/workout/weight data instead of getting a brand-new empty
# row from the default clerk-verify code path (which only matches by
# clerk_user_id, not by username).
#
# Guarded by the ADMIN_MIGRATION_KEY env var: if the var isn't set the
# endpoints are inert (503). If set, the caller must pass ?key=<value>
# matching it.
# ═══════════════════════════════════════════════════════════════════════════

def _admin_guard():
    """Returns (response, status) to short-circuit on, or None if authorized."""
    admin_key = os.environ.get("ADMIN_MIGRATION_KEY")
    if not admin_key:
...
@app.route("/api/admin/inspect-users")
def admin_inspect_users():
    guard = _admin_guard()
    if guard:
        return guard
    from db import get_conn
    with get_conn() as conn:
        rows = conn.execute(
            "SELECT id, username, clerk_user_id, created_at FROM users ORDER BY id"
        ).fetchall()
        users = []
        for r in rows:
            meal_count = conn.execute(
                "SELECT COUNT(*) AS n FROM meal_logs WHERE user_id = ?", (r["id"],)
            ).fetchone()["n"]
            workout_count = conn.execute(
                "SELECT COUNT(*) AS n FROM workout_logs WHERE user_id = ?", (r["id"],)
            ).fetchone()["n"]
            users.append({
                **dict(r),
...
@app.route("/api/admin/migrate-clerk-link")
def admin_migrate_clerk_link():
    guard = _admin_guard()
    if guard:
        return guard

    target_username = request.args.get("target_username", "").strip().lower()
    clerk_user_id = request.args.get("clerk_user_id", "").strip()
    confirm = request.args.get("confirm", "").lower() in ("true", "1", "yes")

    if not target_username:
        return jsonify({"error": "target_username query param required"}), 400
    if not clerk_user_id:
        return jsonify({"error": "clerk_user_id query param required"}), 400

    from db import get_conn
    with get_conn() as conn:
        target = conn.execute(
            "SELECT id, username, clerk_user_id, created_at FROM users WHERE username = ?",
            (target_username,),
        ).fetchone()
        if not target:
        ...
        conflicts = conn.execute(
            "SELECT id, username, clerk_user_id, created_at FROM users "
        ...
        for c in conflicts:
            meal_count = conn.execute(
                "SELECT COUNT(*) AS n FROM meal_logs WHERE user_id = ?", (c["id"],)
            ).fetchone()["n"]
            workout_count = conn.execute(
                "SELECT COUNT(*) AS n FROM workout_logs WHERE user_id = ?", (c["id"],)
            ).fetchone()["n"]
        ...
        if not confirm:
            return jsonify({
                ...
                "target_clerk_id_currently": target_dict.get("clerk_user_id"),
                "conflicting_users": conflict_dicts,
                "would_set_target_clerk_id_to": clerk_user_id,
                "would_delete_user_ids": [c["id"] for c in conflicts],
                "hint": "pass &confirm=true to apply",
            })

        # Apply atomically: update target, delete any conflicting duplicates.
        conn.execute(
            "UPDATE users SET clerk_user_id = ? WHERE id = ?",
            (clerk_user_id, target["id"]),
        )
        for c in conflicts:
            conn.execute("DELETE FROM users WHERE id = ?", (c["id"],))
        conn.commit()

        final = conn.execute(
            "SELECT id, username, clerk_user_id, created_at FROM users WHERE id = ?",
            (target["id"],),
        ).fetchone()

    return jsonify({
        "mode": "applied",
        "target_user_after": dict(final),
        "deleted_user_ids": [c["id"] for c in conflicts],
    })
